...
def transform_speciality(data: pd.DataFrame, table_name: str) -> pd.DataFrame:
    """
    ...
    try:
        ...
        data = data.rename(columns={'speciality_id': 'speciality_nk'})  
        
        # deduplication based on customer_nk
        data = data.drop_duplicates(subset='speciality_nk')
