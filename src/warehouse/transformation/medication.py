...
def transform_medication(data: pd.DataFrame, table_name: str) -> pd.DataFrame:
    """
    ...
    try:
        ...
        data = data.rename(columns={'medication_id': 'medication_nk'})
        
        # deduplication based on customer_nk
        data = data.drop_duplicates(subset='medication_nk')
